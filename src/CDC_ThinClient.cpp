...
double CDC_ThinClient::ThinClient_Add(TCDC_ThinClient& src)
{
	...
	try
	{
		...
		_stmt = compileStatement("insert into CDC_ThinClient values (?, ?, ?, ?, ?, ?, ?, ?, ?);");
...
int CDC_ThinClient::ThinClient_Del(double id)
{
	try
	{
		...
		_stmt = compileStatement("delete from CDC_ThinClient where ThinClient_ID = ?;");
		_stmt.bind(1, id);
		_stmt.execDML();
		_stmt.reset();
	}
...
int CDC_ThinClient::ThinClient_Update(TCDC_ThinClient& src)
{
	try
	{
		...
		_stmt = compileStatement("update CDC_ThinClient \
...
int CDC_ThinClient::ThinClient_Count()
{
	try
	{
		return execScalar("select count(*) from CDC_ThinClient;");
	}
	catch (CppMySQLException& e)
	{
		MERR << e.errorCode() << ":" << e.errorMessage();
...
std::list<TCDC_ThinClient> CDC_ThinClient::GetAll()
{
	...
	try
	{
		CppMySQLQuery q = execQuery("select * from CDC_ThinClient;");
		while (!q.eof())
		{
			TCDC_ThinClient t;
			t.ThinClient_ID = q.getDoubleField(0);
