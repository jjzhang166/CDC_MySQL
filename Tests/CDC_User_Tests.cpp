...
void CDC_User_Tests::testJsonFind()
{
	...
	User1.User_LoginName = "哈哈哈11";
	User1.User_Password = "testJsonFind111";
	User1.User_Remarks = "备注dsds";
	id = _pObj->User_Add(User1);
	assert(id > 0);
	...
	cJSON_AddStringToObject(json, "User_LoginName", "哈哈哈222");
	out = cJSON_Print(json);
	req = out;
	cJSON_Delete(json);
...
void CDC_User_Tests::testJsonFindCount()
{
	...
	cJSON_AddStringToObject(json, "User_LoginName", "哈哈哈11");
	out = cJSON_Print(json);
	req = out;
	cJSON_Delete(json);
